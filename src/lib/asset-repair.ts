...
const PERPLEXITY_PATH = 'M22.3977 7.0896h-2.3106V.0676l-7.5094 6.3542V.1577h-1.1554v6.1966L4.4904 0v7.0896H1.6023v10.3976h2.8882V24l6.932-6.3591v6.2005h1.1554v-6.0469l6.9318 6.1807v-6.4879h2.8882V7.0896zm-3.4657-4.531v4.531h-5.355l5.355-4.531zm-13.2862.0676 4.8691 4.4634H5.6458V2.6262zM2.7576 16.332V8.245h7.8476l-6.1149 6.1147v1.9723H2.7576zm2.8882 5.0404v-3.8852h.0001v-2.6488l5.7763-5.7764v7.0111l-5.7764 5.2993zm12.7086.0248-5.7766-5.1509V9.0618l5.7766 5.7766v6.5588zm2.8882-5.0652h-1.733v-1.9723L13.3948 8.245h7.8478v8.087z';

const BRAND_LOGOS: BrandLogo[] = [
  {
    keys: ['chatgpt', 'openai', 'fchatgpt'],
    label: 'ChatGPT',
    svg: simpleIconSvg('OpenAI', OPENAI_PATH, '#10A37F'),
  },
  {
    keys: ['claude', 'anthropic'],
    label: 'Claude',
    svg: simpleIconSvg('Claude', CLAUDE_PATH, '#D97757'),
  },
  {
    keys: ['gemini', 'google'],
    label: 'Gemini',
    svg: simpleIconSvg('Google Gemini', GEMINI_PATH, '#8E75B2'),
  },
  {
    keys: ['perplexity'],
    label: 'Perplexity',
    svg: simpleIconSvg('Perplexity', PERPLEXITY_PATH, '#1FB8CD'),
  },
];

const BROKEN_RELATIVE_IMAGE_RE =
  /^(?:\.{0,2}\/)?(?:logo|logos|assets|images|img|icons)\//i;
const ASSET_INTENT_RE =
  /(logo|图标|图片|image|img|icon|破图|缺失|加载不出来|没有加载|糊了|补全|补充|修复|替换|换掉|换成|错|不对|错误|不正确|正确|官方|品牌|replace|wrong|correct)/i;
const FORCE_REPLACE_RE =
  /(替换|换掉|换成|换为|改成|错|不对|错误|不正确|正确|官方|品牌|糊了|replace|wrong|correct|official|brand)/i;
...
export function repairAssetsForIntents(
  ...
  const forceReplace = shouldForceReplaceAssets(intents);
  let changed = 0;
  const touched: string[] = [];
  const replacements: Array<{ index: number; from: string; to: string }> = [];
  let srcIndex = 0;
  ...
  $('img').each((_idx, node) => {
    ...
    if (rawSrc === undefined) return;
    const imgSrcIndex = srcIndex++;
    const src = rawSrc.trim();
    const label = inferImageLabel($, node);
    const logo = logoFor(src, label, candidateScope);
    if (!logo) return;
    const nextSrc = svgDataUri(logo.svg);
    if (src === nextSrc) return;
    if (!isLikelyBrokenAsset(src, logo) && !forceReplace) return;

    replacements.push({ index: imgSrcIndex, from: src, to: nextSrc });
    changed++;
    touched.push(logo.label);
  });

  if (changed === 0) {
    return { ok: false, reason: 'no replaceable logo/image source matched known assets' };
  }
  ...
  return {
    ...
    changed,
    reason: `asset repair/replacement updated ${changed} image src(s): ${dedupe(touched).join(', ')}`,
  };
}
...
function shouldRepairAssets(intents: Intent[]): boolean {
  return intents.some(intent => ASSET_INTENT_RE.test(intent.statement));
}

function shouldForceReplaceAssets(intents: Intent[]): boolean {
  const text = candidateScopeText(intents);
  return /(logo|图标|图片|image|img|icon)/i.test(text) && FORCE_REPLACE_RE.test(text);
}
...
function logoFor(src: string, label: string, scope: string): BrandLogo | null {
  const localHaystack = normalize(`${src} ${label}`);
  const localMatch = BRAND_LOGOS.find(logo =>
    logo.keys.some(key => localHaystack.includes(key))
  );
  if (localMatch) return localMatch;

  const scopeMatches = BRAND_LOGOS.filter(logo =>
    logo.keys.some(key => scope.includes(key))
  );
  if (scopeMatches.length === 1 && /(logo|icon|图标|brand|mark)/i.test(localHaystack)) {
...
function simpleIconSvg(title: string, path: string, fill: string): string {
  return `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>${title}</title><path fill="${fill}" d="${path}"/></svg>`;
}
...
function replaceImgSrcValues(
  ...
  let out = html;
  const byIndex = new Map(replacements.map(item => [item.index, item]));
  let imgIndex = 0;
  out = out.replace(
    /(<img\b[^>]*?\ssrc\s*=\s*)(["'])(.*?)\2/gi,
    (full, prefix: string, quote: string, value: string) => {
      const replacement = byIndex.get(imgIndex++);
      if (!replacement || value.trim() !== replacement.from) return full;
